...
public class RoleInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        try {
            ...
            if (path.matches("^/[^/]+/admin/.*$") && !apiResponse.getMessage().equals("Admin")) {
            ...
            System.err.println("Xác thực thành công!");
            return true;
        } catch (IOException e) {
            // Nếu có lỗi trong quá trình verify (ví dụ token lỗi, mạng lỗi,…)
            ApiResponse<Object> apiResponse = ApiResponse.builder()
                    .code(HttpServletResponse.SC_BAD_GATEWAY)
                    .message("Xác thực thỉnh thoảng lỗi hoặc hết hạn: " + e.getMessage())
                    .result(null)
                    .build();
